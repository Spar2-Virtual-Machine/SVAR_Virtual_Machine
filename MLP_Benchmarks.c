...
void MLP_VM_For64x64_Time(AllocationTable *table){
	...
	Store_V(&in, 2, table); //Store input
	Store_M(&w1, 1, table); //Store weight1
	Mul_MV(1, 2, 3, table); //Multiply input by weight1

	Store_V(&b1, 4, table); //Store the bias vector
	E_Add_VV(3, 4, 3, table); //Add the bias

	Store_M(&w2, 5, table); //Store the weight
	Mul_MV(5, 3, 6, table); //Multiply by the weight
	Store_V(&b2, 7, table); //Store the bias
	E_Add_VV(6, 7, 6, table); //Add the bias
	XTime_GetTime(&tEnd);
...
void MLP_Native_For64x64_Count(AllocationTable *table){
	...
	//fill all 3 physical registers for the vectors
	MATRIX_SUBTRACTION(1, 1, 0);
	SHIFT_EAST(17, 0);
	for(int x=0; x<SPAR_dimension-1; x++)
	{
...
void MLP_Native_For64x64_Time(AllocationTable *table){
	...
	XTime_GetTime(&tStart);
	//write Matrix Into Memory
	WRITE_Matrix_Large(l1Nodes, inputSize, 0, 0, 64, 64, weight1, 1, 1, 0);
	WRITE_Matrix_Large(l1Nodes, inputSize, 0, 1, 64, 64, weight1, 2, 1, 0);
	WRITE_Matrix_Large(l1Nodes, inputSize, 1, 0, 64, 64, weight1, 3, 1, 0);
	WRITE_Matrix_Large(l1Nodes, inputSize, 1, 1, 64, 64, weight1, 4, 1, 0);
	WRITE_Matrix_Large(l1Nodes, inputSize, 2, 0, 64, 64, weight1, 5, 1, 0);
	WRITE_Matrix_Large(l1Nodes, inputSize, 2, 1, 64, 64, weight1, 6, 1, 0);
	...
	//load in the newest weight matrix
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 0, 0, 64, 64, weight2, 1, 2, 0);
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 0, 1, 64, 64, weight2, 2, 2, 0);
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 0, 2, 64, 64, weight2, 3, 2, 0);
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 1, 0, 64, 64, weight2, 4, 2, 0);
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 1, 1, 64, 64, weight2, 5, 2, 0);
	WRITE_Matrix_Large(l2Nodes, l1Nodes, 1, 2, 64, 64, weight2, 6, 2, 0);
